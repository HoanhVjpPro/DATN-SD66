...
public class UserController {
    ...
    @ResponseBody
    public Map<String, Object> checkField(@RequestParam String field,
                                          @RequestParam String value) {
        Map<String, Object> result = new HashMap<>();

        boolean available = switch (field) {
            case "username" -> !userRepository.existsByUsername(value.trim());
            case "email"    -> !userRepository.existsByEmail(value.trim());
            case "phone"    -> !userRepository.existsByPhone(value.trim());
            default         -> true;
        };
